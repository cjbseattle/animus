import { useGetMeStats, useListQuestions } from "@workspace/api-client-react";
import { Activity, Target, BrainCircuit, BookOpen, Clock } from "lucide-react";
...
export default function Stats() {
  ...
  const { data: recentQuestions, isLoading: isLoadingRecent } = useListQuestions({ limit: 5 });
    ...
    <div className="flex-1 flex flex-col pt-8 pb-24 px-6 overflow-y-auto">
      ...
      {isLoading ? (
        ...
        <div className="space-y-6">
          ...
          <h2 className="text-sm font-mono text-muted-foreground uppercase tracking-widest mt-8 mb-2 flex items-center gap-2">
          ...
          {isLoadingRecent ? (
            ...
            <div className="space-y-3">
              {recentQuestions?.map((q) => (
                <div key={q.id} className="p-4 rounded-xl border border-border bg-card/50 flex flex-col gap-2">
